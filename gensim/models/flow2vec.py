...
# Modified by Zhou Changling< zcl@pku > 2016
...
>>> model = Flow2Vec(sentences, size=100, window=5, min_count=5)

Persist a model to disk with::

>>> model.save(fname)
>>> model = Flow2Vec.load(fname)  # can continue training with the loaded model!

The model can also be instantiated from an existing file on disk in the flow2vec C format::

  >>> model = Flow2Vec.load_flow2vec_format('/tmp/vectors.txt', binary=False)  # text format
  >>> model = Flow2Vec.load_flow2vec_format('/tmp/vectors.bin', binary=True)  # binary format
...
logger = logging.getLogger("gensim.models.flow2vec")
...
try:
    ...
    from flow2vec_inner import train_sentence, FAST_VERSION
except:
    ...
    def train_sentence(model, sentence, alpha):
        ...
        word is not in the vocabulary). Called internally from `Flow2Vec.train()`.

        """
        for pos, word in enumerate(sentence):
            if word is None:
                continue  # OOV word in the input sentence => skip
            reduced_window = random.randint(model.window)  # `b` in the original flow2vec code
...
class Flow2Vec(utils.SaveLoad):
    ...
    compatible with the original flow2vec implementation via `save_flow2vec_format()`.
    ...
    def save_flow2vec_format(self, fname, binary=False):
        """
    ...
    @classmethod
    def load_flow2vec_format(cls, fname, binary=False):
        """
        ...
        with open(fname) as fin:
            ...
            result = Flow2Vec(size=layer1_size)
    ...
    def most_similar(self, positive=[], negative=[], topn=10):
        ...
        `distance` scripts in the original flow2vec implementation.
...
if __name__ == "__main__":
    ...
    w = Flow2Vec(LineSentence(infile), size=200, min_count=5)
    w.save(outfile + '.model')
    w.save_flow2vec_format(outfile + '.model.bin', binary=True)
    w.save_flow2vec_format(outfile + '.model.txt', binary=False)
