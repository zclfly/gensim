...
# Modified by Zhou Changling@PKU, 2016


"""
Deep learning via flow2vec's "skip-gram and CBOW models", using either
hierarchical softmax or negative sampling [1]_ [2]_.

The training algorithms were originally ported from the C package https://code.google.com/p/word2vec/
and extended with additional functionality.

For a blog tutorial on gensim flow2vec, with an interactive web app trained on GoogleNews, visit http://radimrehurek.com/2014/02/word2vec-tutorial/

**Make sure you have a C compiler before installing gensim, to use optimized (compiled) flow2vec training**
(70x speedup compared to plain NumPy implementation [3]_).

Initialize a model with e.g.::

>>> model = Flow2Vec(sentences, size=100, window=5, min_count=5, workers=4)

Persist a model to disk with::

>>> model.save(fname)
>>> model = Flow2Vec.load(fname)  # you can continue training with the loaded model!

The model can also be instantiated from an existing file on disk in the flow2vec C format::

  >>> model = Flow2Vec.load_flow2vec_format('/tmp/vectors.txt', binary=False)  # C text format
  >>> model = Flow2Vec.load_flow2vec_format('/tmp/vectors.bin', binary=True)  # C binary format
...
detect phrases longer than one word. Using phrases, you can learn a flow2vec model
where "words" are actually multiword expressions, such as `new_york_times` or `financial_crisis`:

>>> bigram_transformer = gensim.models.Phrases(sentences)
>>> model = Flow2Vec(bigram_transformer[sentences], size=100, ...)
...
.. [3] Optimizing flow2vec in gensim, http://radimrehurek.com/2013/09/flow2vec-in-python-part-two-optimizing/
"""
...
try:
    from gensim.models.flow2vec_inner import train_batch_sg, train_batch_cbow
    from gensim.models.flow2vec_inner import score_sentence_sg, score_sentence_cbow
    from gensim.models.flow2vec_inner import FAST_VERSION, MAX_WORDS_IN_BATCH
except ImportError:
    ...
    def train_batch_sg(model, sentences, alpha, work=None):
        ...
        vocab dictionary. Called internally from `Flow2Vec.train()`.

        This is the non-optimized, Python version. If you have cython installed, gensim
        will use the optimized version from flow2vec_inner instead.
    ...
    def score_sentence_sg(model, sentence, work=None):
        ...
        word is not in the vocabulary). Called internally from `Flow2Vec.score()`.
...
class Flow2Vec(utils.SaveLoad):
    ...
    def train(self, sentences, total_words=None, word_count=0,
        ...
        For Flow2Vec, each sentence must be a list of unicode strings. (Subclasses may accept other examples.)
        ...
        if FAST_VERSION < 0:
            import warnings
            warnings.warn("C extension not loaded for Flow2Vec, training will be slow. "
    ...
    def score(self, sentences, total_sentences=int(1e6), chunksize=100, queue_factor=2, report_delay=1):
        ...
        Each sentence must be a list of unicode strings.
        This does not change the fitted model in any way (see Flow2Vec.train() for that)
    ...
    def save_flow2vec_format(self, fname, fvocab=None, binary=False):
        """
        Store the input-hidden weight matrix in the same format used by the original
        C flow2vec-tool, for compatibility.
    ...
    @classmethod
    def load_flow2vec_format(cls, fname, fvocab=None, binary=False, encoding='utf8', unicode_errors='strict'):
        """
        Load the input-hidden weight matrix from the original C flow2vec-tool format.

        Note that the information stored in the file is incomplete (the binary tree is missing),
        so while you can query for word similarity etc., you cannot continue training
        with a model loaded this way.

        `binary` is a boolean indicating whether the data is in binary flow2vec format.
        `norm_only` is a boolean indicating whether to only store normalised flow2vec vectors in memory.
    ...
    def intersect_flow2vec_format(self, fname, binary=False, encoding='utf8', unicode_errors='strict'):
        """
        Merge the input-hidden weight matrix from the original C flow2vec-tool format
    ...
    def accuracy(self, questions, restrict_vocab=30000, most_similar=most_similar):
        ...
        See https://code.google.com/p/flow2vec/source/browse/trunk/questions-words.txt for an example.
        ...
        This method corresponds to the `compute-accuracy` script of the original C flow2vec.
    ...
    def save(self, *args, **kwargs):
        ...
        super(Flow2Vec, self).save(*args, **kwargs)

    save.__doc__ = utils.SaveLoad.save.__doc__

    @classmethod
    def load(cls, *args, **kwargs):
        model = super(Flow2Vec, cls).load(*args, **kwargs)
        # update older models
...
# Example: ./flow2vec.py -train data.txt -output vec.txt -size 200 -window 5 -sample 1e-4 -negative 5 -hs 0 -binary 0 -cbow 1 -iter 3
if __name__ == "__main__":
    ...
    from gensim.models.flow2vec import Flow2Vec  # avoid referencing __main__ in pickle
    ...
    model = Flow2Vec(corpus, size=args.size, min_count=args.min_count, workers=args.threads, window=args.window,sample=args.sample,sg=skipgram,hs=args.hs,negative=args.negative,cbow_mean=1,iter=args.iter)

    if args.output:
        outfile = args.output
        model.save_flow2vec_format(outfile, binary=args.binary)
    else:
        outfile = args.train
        model.save(outfile + '.model')
    if args.binary == 1:
        model.save_flow2vec_format(outfile + '.model.bin', binary=True)
    else:
        model.save_flow2vec_format(outfile + '.model.txt', binary=False)
